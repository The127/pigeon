...
#[async_trait]
impl QueryHandler<GetEventTypeStats> for S {
    async fn handle(&self, _: GetEventTypeStats) -> Result<EventTypeStats, ApplicationError> {
        Ok(EventTypeStats {
            total_messages: 0,
            total_attempts: 0,
            total_pending: 0,
...
#[async_trait]
impl QueryHandler<GetEndpointStats> for S {
    async fn handle(&self, _: GetEndpointStats) -> Result<EndpointStats, ApplicationError> {
        Ok(EndpointStats {
            total_attempts: 0,
            total_pending: 0,
            total_succeeded: 0,
...
fn test_state() -> AppState {
    AppState {
        ...
        get_event_type_stats: Arc::new(S),
        get_endpoint_stats: Arc::new(S),
