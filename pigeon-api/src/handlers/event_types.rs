...
pub(crate) async fn create_event_type(
    ...
    let et = dispatch(state.create_event_type.clone(), command, &auth.user_id, &auth.org_id, state.uow_factory.clone(), state.audit_store.clone()).await.map_err(ApiError)?;
...
pub(crate) async fn update_event_type(
    ...
    let et = dispatch(state.update_event_type.clone(), command, &auth.user_id, &auth.org_id, state.uow_factory.clone(), state.audit_store.clone()).await.map_err(ApiError)?;
...
pub(crate) async fn delete_event_type(
    ...
    dispatch(state.delete_event_type.clone(), command, &auth.user_id, &auth.org_id, state.uow_factory.clone(), state.audit_store.clone()).await.map_err(ApiError)?;
...
mod tests {
    ...
    impl CommandHandler<CreateEventType> for FakeCreateEtHandler {
        async fn handle(&self, command: CreateEventType, _ctx: &mut RequestContext) -> Result<EventType, ApplicationError> {
            match &self.result {
    ...
    impl CommandHandler<DeleteEventType> for FakeDeleteEtHandler {
        async fn handle(&self, _command: DeleteEventType, _ctx: &mut RequestContext) -> Result<(), ApplicationError> {
            match &self.result {
                Ok(()) => Ok(()),
                Err(e) => Err(match e {
                    ApplicationError::NotFound => ApplicationError::NotFound,
