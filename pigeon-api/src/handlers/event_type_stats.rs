...
pub(crate) struct EventTypeStatsQuery {
...
pub(crate) async fn get_event_type_stats(
...
mod tests {
    ...
    #[tokio::test]
    async fn get_event_type_stats_returns_200() {
        let app_id = pigeon_domain::application::ApplicationId::new();
        let event_type_id = pigeon_domain::event_type::EventTypeId::new();
        ...
        let response = router
            .oneshot(
                Request::builder()
                    .method("GET")
                    .uri(&format!(
                        "/api/v1/applications/{}/event-types/{}/stats",
                        app_id.as_uuid(),
                        event_type_id.as_uuid()
                    ))
