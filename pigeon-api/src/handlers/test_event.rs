...
pub(crate) async fn send_test_event(
    ...
    let result = dispatch(state.send_test_event.clone(), command, &auth.user_id, &auth.org_id, state.uow_factory.clone(), state.audit_store.clone()).await.map_err(ApiError)?;
